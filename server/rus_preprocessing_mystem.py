...
def tag_mystem(text='Текст нужно передать функции в виде строки!',
    ...
    for w in processed:
        try:
            ...
            normtag = lemma.lower() + '_' + pos
        except (KeyError, IndexError):
            # KeyError - пунктуация и пробелы ('analysis' not in w),
            # IndexError - если нет норм формы,
            # такое бывает для слов латинницей, например
            # print(w)
            normtag = ''
        clean = w['text']
        tokens.append(clean)
